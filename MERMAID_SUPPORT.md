...
| Class | `classDiagram` | ✅ | classes w/ properties/methods; **now**: `<<interface/abstract/enumeration>>` stereotypes, generics `~T~`→`<T>` (generic class ids unified), `direction`, multiplicities `"1" o-- "0..*"`, all relation operators (`<\|--`, `..\|>`, `*--`, `o--`, `-->`, `..>`), dotted relations dashed, auto-size, **UML end-markers** (hollow triangle = inheritance/realization, filled/hollow diamond = composition/aggregation, open arrow = association/dependency, at the correct end), connection-aware non-overlapping layout, **namespaces** (`namespace Foo { … }` → grouping container holding its classes, #27). TODO: nested namespaces |
| State | `stateDiagram` / `-v2` | 🟡 | states→rounded rects, transitions+labels, `[*]`→small dot. **Now (#22):** composite `state X { … }`→container (inner states nested via subgraphs), `<<fork>>/<<join>>`→thin bar, `<<choice>>`→diamond, `note left/right of X`→linked note node (dashed; single- or multi-line). TODO: concurrency regions `--`, `direction` inside composite |
...
| Mindmap | `mindmap` | 🟡 | indentation-based tree → nodes + parent/child edges; shapes `((circle))`/`(round)`/`[square]`/`{{hex→rect}}`. **Now (#24):** **radial layout** (root centered, depth→ring radius, siblings spread by subtree leaf-count so they don't overlap) + `::icon(fa fa-x)`→node icon. TODO: class styling, collision-perfect packing for very large trees |
| Timeline | `timeline` | 🟡 | `period : event…` → period (pill) with event (rect) children; `: event` continues a period. **Now (#29):** **horizontal** layout (periods left→right, events stacked below each), `title`→heading node, and per-`section` palette tint on the period pills (fixed geometry, layout passes it through). TODO: curved/zoomable time axis |
| Gantt | `gantt` | 🟡 | tasks→horizontal bars on a 20px/day axis (start date → x, duration → width); `after <id>` deps resolve start; sections→left labels; milestones→diamonds. **Now (#25):** dependency **arrows** (predecessor→`after` task), a **weekly date-axis** (tick label every 7 days across the range), and a **today marker** when in range. TODO: weekends/excludes shading |
| Pie | `pie` | 🟡 | one `pie` node renders an actual pie (slice `<path>` arcs) + legend with values & percentages; slice colors from the palette. **Now (#30):** `node.donut` flag switches slices to **annulus** (inner-hole) paths without changing the parse; **leader lines** connect each slice to a % label at the rim. TODO: multi-ring/nested pies, exploded slices |
| User Journey | `journey` | 🟡 | tasks→points placed left-to-right, vertical position = satisfaction score (1–5), chained into the journey line. **Now (#31):** each task renders **color-coded actor avatar glyphs** (initial in a tinted circle, one per actor, stable color per actor) instead of naming them in the label; each `section` draws a tinted **background band** spanning its tasks. TODO: configurable avatar images |
| Git graph | `gitGraph` | 🟡 | commits→dots on per-branch lanes (left→right), `branch` forks from current tip, `merge` joins a branch tip into a new 2-parent commit; dots colored per lane, labeled when an explicit id is set; lane labels. **Now (#26):** `commit tag: "v1.0"`→tag label above the dot, `commit type: HIGHLIGHT`→thick accent ring / `REVERSE`→✗ overlay, `cherry-pick id: "X"`→dashed link from source commit X. TODO: TB (top-bottom) direction |
...
| Requirement | `requirementDiagram` | 🟡 | requirements & elements → class-style boxes (type as «stereotype», attrs as rows); typed links (satisfies/traces/contains/derives/refines/verifies/copies) → labeled edges (dashed except contains). **Now (#32):** `contains` → a **filled diamond** (SysML composition) at the parent end, solid line; also handles inline single-line `{ … }` blocks. TODO: verification-method shapes |
| C4 | `C4Context` etc. | 🟡 | Person/System/Container/Component/Node (incl. _Ext/Db/Queue) → class-style boxes (type as «stereotype», tech/descr wrapped rows); `Rel(...)`→labeled edges (BiRel→both ends, tech appended); `*_Boundary(){}`→grouping containers. **Now (#28):** directional `Rel_U/D/L/R` (and _Up/_Down/_Left/_Right) set + lock the connection sides so the layout honours the direction; `Person(...)` boxes render a person-figure icon. TODO: dynamic/deployment variants, sprite/tech icons |
