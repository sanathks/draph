...
| Quadrant | `quadrantChart` | 🟡 | one `quadrant` node renders a 2×2 plot: tinted quadrants + labels, center cross, x/y axis labels, and plotted points (x,y 0..1, y up). **Now (#33):** per-point inline styling — `Pt: [x,y] radius: N, color: #hex, stroke-color: #hex, stroke-width: N` (and `:::class` stripped from the label); render honours per-point radius/color/stroke. TODO: classDef cascade, legend |
